...
export default function AppShell({ children }: { children: React.ReactNode }) { const pathname = usePathname(); const capture = pathname.startsWith('/o/'); if (capture) return <>{children}</>; return <><header className="app-header"><div className="app-header-inner"><Link href="/" className="brand"><img src="/icon-192.png" alt="Eolas logo" />EOLAS <span>Admin</span></Link><span className="app-version" title={`commit ${process.env.NEXT_PUBLIC_COMMIT_SHA}`}>v{process.env.NEXT_PUBLIC_APP_VERSION}</span><WorkspaceSwitcher /><AppNav /><ThemeToggle /></div></header>{children}<Dock /></>; }
