...
export async function POST(request: NextRequest, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    const project = await prisma.project.findUnique({ where: { id: params.id }, include: { assemblyPlans: { where: { status: 'COMPLETED' }, orderBy: { completedAt: 'desc' }, take: 1 } } });
